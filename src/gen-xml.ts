...
/**
 * Build an INLINE native-equation run (sf-inline-math) from raw OMML.
 *
...
export function genXmlTextBody(slideObj: SlideObject | TableCell): string {
	...
	*/
	...
	arrTextObjects.forEach((textObj, idx) => {
		...
		// Inline math (sf-inline-math) flows mid-paragraph, so it falls through to the run-buffering
	...
	arrLines.forEach((line) => {
		...
		line.forEach((textObj, idx) => {
			...
			if (_textRunObj.math) {
				// Inline native equation (sf-inline-math): a bare <a14:m><m:oMath> run between plain runs.
