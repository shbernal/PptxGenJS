...
- **Feature serialization**: `bar-chart-data-labels.pptx` (per-point bar
  ...
  **inline**, in-sentence equation `a14:m`/`m:oMath` with no `m:oMathPara`, flowing
  between plain `a:r` runs in one `a:p` — backlog `sf-inline-math`).
- **A/V media rel graph**: `av-media.pptx` (embedded video + audio rel/content-type
  oracle for `appendSlides`, backlog `sf-append-av-media`); `online-video.pptx`
...
generator path in `appendSlides` tests (backlog `sf-append-av-media`). Encoded
...
- `multi-theme.pptx` — a minimal Ion-theme deck. **Slide 1** carries
  ...
  `sf-importslide-restyle-literals`): `literal-accent1`, a rectangle whose fill is
...
- `template.potx` — a genuine PowerPoint-authored **`.potx` template** package
  ...
  `.pptx` (backlog `sf-import-template-masters`). Carries the default Office theme
...
- `math-omml-inline.pptx` — **authoring oracle** for an **inline** (in-sentence)
  equation run — the write-side gate for backlog `sf-inline-math` (the sibling of
  ...
  *substring* of a paragraph that also carries plain text, so Word makes it inline
  ...
  — the open question `sf-inline-math` was gated on:
...
- `av-media.pptx` — **authoring oracle** for embedded audio/video in appended
  slides (backlog `sf-append-av-media`). Two blank slides, each with one media
...
- `online-video.pptx` — **authoring oracle** for online (external-link) video in
  appended slides (backlog `sf-append-online-video`). One blank slide with a single
...
- `import-animation-merge.pptx` — **authoring oracle** for carrying a build animation through
  ...
  `sf-importshape-v1-limits`). Two blank 16:9 slides. **Slide 1** `Source` (spid 2) has an
...
(`docs/measured-text-fit.md`, backlog `sf-measured-text-fit`). Like the authoring
